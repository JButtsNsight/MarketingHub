#!/usr/bin/env bash
# =============================================================================
# deploy-prod.sh — the MarketingHub PRODUCTION CUTOVER / prod deploy
#
# Derived from deploy-preview.sh (same live-derivation + no-silent-strip
# doctrine); deploys the SAME stack with previewMode OFF: internet-facing ALB
# + HTTPS/ACM + WAF + Route53 alias + authenticate-cognito bound to the
# PRESTAGED pool us-east-1_EILGYZVyA. The Cognito broker synths with the same
# logical IDs it was prestaged with — it MUST be a NO-OP in the diff; a
# replace/delete there breaks the live Google-side SAML registration = STOP.
#
...
# Prod front-door identity — pinned; the prestaged broker + Google-side SAML
# registration are bound to exactly these.
APP_HOSTNAME=marketinghub.nsightcare.com
EXPECTED_POOL_ID=us-east-1_EILGYZVyA
EXPECTED_POOL_CLIENT=31tiiev4n5sub0qk1resai59e8
...
case "$ALB_SCHEME" in
  internal)        echo ">> live front door is INTERNAL (preview) — this run CUTS OVER to the public front door" ;;
  internet-facing) echo ">> live front door is already INTERNET-FACING — routine prod deploy" ;;
  *) fail "unrecognized live ALB scheme '$ALB_SCHEME'" ;;
esac
...
elif [ -n "$LIVE_SMS_LINK" ] && [ "${LIVE_SMS_LINK#http://localhost}" = "$LIVE_SMS_LINK" ]; then
  SMS_LINK="$LIVE_SMS_LINK"; SMS_LINK_SRC="live task-def"
else
  # The preview tunnel URL must never reach prod tracked links.
  SMS_LINK="https://$APP_HOSTNAME"; SMS_LINK_SRC="prod canonical"
fi
...
[ -f "$SAML_METADATA_FILE" ] \
  || fail "PROD requires the Google IdP metadata XML at $SAML_METADATA_FILE — the Cognito front door cannot synth without it"

# Prod-only DNS context, derived live from Route53 (delegated subdomain zone).
ZONE_ID=$(aws route53 list-hosted-zones-by-name --dns-name "$APP_HOSTNAME" \
  --query "HostedZones[?Name=='${APP_HOSTNAME}.'].Id | [0]" --output text)
{ [ -n "$ZONE_ID" ] && [ "$ZONE_ID" != "None" ]; } || fail "no public hosted zone for $APP_HOSTNAME"
ZONE_ID=${ZONE_ID##*/}
CDK_CTX+=(
  -c hostedZoneId="$ZONE_ID"
  -c hostedZoneName="$APP_HOSTNAME"
)
...
# PREVIEW_AUTH is DROPPED BY DESIGN in prod (real ALB OIDC identity replaces
# the shim) — the single allowed app-container loss; anything else aborts.
APP_LOSS=$(jq '[.missing_env[] | select(. != "PREVIEW_AUTH")] + .missing_secrets | length' <<<"$APP_REPORT")
WORKER_LOSS=$(jq '.missing_env + .missing_secrets | length' <<<"$WORKER_REPORT")
if jq -e '.missing_env | index("PREVIEW_AUTH")' <<<"$APP_REPORT" >/dev/null; then
  echo ">> app drops PREVIEW_AUTH — EXPECTED (prod uses real ALB OIDC identity)"
fi
...
cat <<SUMMARY
  ...
  ---------------------------------------------------------------------------
  PROD DEPLOY PLAN — everything below was derived LIVE just now
  ---------------------------------------------------------------------------
  stack                     $STACK  (previewMode=OFF; live scheme=$ALB_SCHEME)
  front door                https://$APP_HOSTNAME  (ALB + ACM + WAF + Route53 + Cognito)
  cognito broker (pinned)   $EXPECTED_POOL_ID / $EXPECTED_POOL_CLIENT — must be a NO-OP
  ...
  the CUTOVER: ALB replaced internal -> internet-facing, HTTPS listener + ACM
  cert + WAF + Route53 alias ADDED, task-def churn (PREVIEW_AUTH dropped;
  ALB_ARN / COGNITO_LOGOUT_URL added). The Cognito broker (AppUserPool /
  GoogleSamlIdp / AppClient / AppUserPoolDomain) MUST show NO changes — a
  replace/delete there breaks the live Google SAML registration = STOP.
  ---------------------------------------------------------------------------
...
# PREVIEW_AUTH loss is the deliberate cutover change — filter it from the check.
APP_LOST=$(printf '%s' "$APP_LOST" | sed -e 's/PREVIEW_AUTH, //' -e 's/, PREVIEW_AUTH//' -e 's/^PREVIEW_AUTH$//')
...
ROLLBACK
  Same-door image rollback (keeps cdk ownership — PREFERRED):
    echo $CONFIRM_WORD | bash app-infra/scripts/deploy-prod.sh $OLD_APP_TAG
  FULL front-door rollback (restores the INTERNAL preview door; tears down
  the public ALB/cert/alias — the Cognito broker stays, it is RETAINed and
  prestaged):
    echo DEPLOY-PREVIEW | bash app-infra/scripts/deploy-preview.sh $OLD_APP_TAG
...
echo "== [8b] PROD post-verify: broker intact + public front door answering ====="
aws cognito-idp describe-user-pool-client --user-pool-id "$EXPECTED_POOL_ID" \
  --client-id "$EXPECTED_POOL_CLIENT" --region "$REGION" >/dev/null \
  || fail "prestaged pool/client ($EXPECTED_POOL_ID / $EXPECTED_POOL_CLIENT) no longer resolves — the broker was REPLACED and the Google-side SAML registration is broken. Investigate IMMEDIATELY."
echo ">> Cognito broker intact: $EXPECTED_POOL_ID / $EXPECTED_POOL_CLIENT"

HEALTH=down
for _ in $(seq 1 30); do
  CODE=$(curl -s -o /dev/null -w '%{http_code}' --max-time 10 "https://$APP_HOSTNAME/api/health" </dev/null || true)
  if [ "$CODE" = "200" ]; then HEALTH=up; break; fi
  sleep 10
done
if [ "$HEALTH" = "up" ]; then
  echo ">> PUBLIC FRONT DOOR UP: https://$APP_HOSTNAME/api/health -> 200"
else
  echo ">> WARN: https://$APP_HOSTNAME/api/health not 200 after ~5 min — DNS/cert propagation can lag; check manually before alarming"
fi
