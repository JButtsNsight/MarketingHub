...
- [ ] Task def env: `SUPABASE_URL` (context), `NEXT_PUBLIC_APP_NAME`, **`ALB_ARN` = the front-door ALB's `loadBalancerArn`, and `AWS_REGION`/`ALB_REGION` = `this.region`** (REQUIRED — the app's ALB JWT signature verification in `web/src/lib/auth.ts`, added as a post-Phase-4 security hardening, is fail-loud: it verifies the `x-amzn-oidc-data` ES256 signature against the ALB public key and asserts the token's `signer` equals `ALB_ARN`, so a token arriving without `ALB_ARN` set throws); secret: `SUPABASE_SERVICE_ROLE_KEY` from Secrets Manager ARN (context `supabaseServiceRoleSecretArn`) via `ecs.Secret.fromSecretsManager`, IAM `grantRead` on the exact ARN (no wildcard). Update `app-stack.test.ts` to assert `SUPABASE_URL`, `ALB_ARN`, the secret, and the ARN-scoped IAM are all present. Green. Commit.

> **Security note (auth):** `getUser`/`requireUser` are async and cryptographically verify the ALB `x-amzn-oidc-data` JWT (ES256, cached public key by `kid`, `signer`+`exp` checks) — we do NOT trust the unverified payload. This was hardened after Phase 4 per a security review; the only new deploy dependency is the `ALB_ARN`/region env above.
